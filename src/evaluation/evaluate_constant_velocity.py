from __future__ import annotations

import argparse
import json
from pathlib import Path
from typing import Any

import numpy as np
from tqdm import tqdm

from src.datasets.datamodule import build_dataloaders
from src.datasets.normalization import deltas_to_positions, load_stats
from src.evaluation.metrics import MetricAccumulator, trajectory_metrics
from src.models.baselines.constant_velocity import predict_constant_velocity
from src.training.train_diffusion import build_data_config, load_yaml


def invert_past_normalization(values: np.ndarray, stats: dict[str, np.ndarray]) -> np.ndarray:
    return values * stats["past_std"] + stats["past_mean"]


def decode_future(values: np.ndarray, stats: dict[str, np.ndarray], representation: str) -> np.ndarray:
    if representation == "delta":
        deltas = values * stats["future_delta_std"] + stats["future_delta_mean"]
        return deltas_to_positions(deltas)
    return values * stats["future_std"] + stats["future_mean"]


def save_report(path: Path, report: dict[str, Any]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", encoding="utf-8") as f:
        json.dump(report, f, indent=2, ensure_ascii=False)


def evaluate(
    config_path: Path,
    fps: float,
    velocity_window: int,
    max_trajectories: int | None,
    report_path: Path,
) -> dict[str, Any]:
    config = load_yaml(config_path)
    data_config = build_data_config(config)
    loaders = build_dataloaders(data_config)
    test_loader = loaders["test"]
    stats = load_stats(data_config.stats_path)
    pred_len = int(config["model"]["pred_len"])

    accumulator = MetricAccumulator()
    processed = 0
    for batch in tqdm(test_loader, desc="constant velocity"):
        if max_trajectories is not None:
            remaining = max_trajectories - processed
            if remaining <= 0:
                break
            if len(batch["past"]) > remaining:
                batch = {key: value[:remaining] for key, value in batch.items()}

        relative_past = invert_past_normalization(batch["past"], stats)
        relative_future = decode_future(batch["future"], stats, data_config.future_representation)
        predictions = predict_constant_velocity(
            relative_past,
            pred_len=pred_len,
            fps=fps,
            velocity_window=velocity_window,
        )
        predictions_xy = predictions + batch["origin"][:, None, :]
        future_xy = relative_future + batch["origin"][:, None, :]
        accumulator.update(trajectory_metrics(predictions_xy[:, None, :, :], future_xy))
        processed += len(batch["past"])

    report = {
        "baseline": "constant_velocity",
        "fps": fps,
        "velocity_window": velocity_window,
        "metrics": accumulator.compute(num_samples=1),
    }
    save_report(report_path, report)
    return report


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate the constant-velocity baseline.")
    parser.add_argument("--config", type=Path, default=Path("configs/ngsim_diffusion.yaml"))
    parser.add_argument("--fps", type=float, default=10.0)
    parser.add_argument("--velocity-window", type=int, default=5)
    parser.add_argument("--max-trajectories", type=int)
    parser.add_argument("--report-path", type=Path, default=Path("outputs/reports/constant_velocity_metrics.json"))
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    report = evaluate(
        config_path=args.config,
        fps=args.fps,
        velocity_window=args.velocity_window,
        max_trajectories=args.max_trajectories,
        report_path=args.report_path,
    )
    print(json.dumps(report, indent=2, ensure_ascii=False))


if __name__ == "__main__":
    main()
